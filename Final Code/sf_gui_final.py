...
"border-radius: 10px;")
        ...
        self.AddItem.clicked.connect(self.AddButtonclicked)
...
"")
        ...
        self.quote()
        timerQuote = QTimer(SmartFridge)
        timerQuote.timeout.connect(self.quote)
        timerQuote.start(60000)
        ...
        self.Quotes.setStyleSheet("background-color: rgb(254, 177, 31);\n"
"alternate-background-color: rgba(50, 51, 51, 66);\n"
"border-radius: 15px;")
...
"")
        ...
        self.ShopList()
        timerShop = QTimer(SmartFridge)
        timerShop.timeout.connect(self.ShopList)
        timerShop.start(60000)
...
"")
        ...
        font.setFamily("Times New Roman")
        font.setPointSize(18)
        self.ShoppingList.setFont(font)
...
"border-radius: 10px;")
        ...
        self.RemoveItem.clicked.connect(self.RemoveButtonclicked)
...
"border-radius: 10px;")
        ...
        self.FreshnessNotification = QtWidgets.QTextEdit(self.frame)
        self.freshness()
        
        timerFresh = QTimer(SmartFridge)
        timerFresh.timeout.connect(self.freshness)
        timerFresh.start(60000)
...
"")
        ...
        font.setFamily("Times New Roman")
        font.setPointSize(18)
        self.FreshnessNotification.setFont(font)
        self.FreshnessNotification.setTextColor(QtGui.QColor('#FF0000'))
...
"border-radius: 10px;")
    ...
    def ShopList(self):
        ...
        cursor.movePosition(QtGui.QTextCursor.End)
        cursor.insertText("----- Shopping List -----")
        ...
        for value in value.each():
            ...
            self.ShoppingList.setTextCursor(cursor)
            self.ShoppingList.ensureCursorVisible()
            #self.ShoppingList.setText(text)
            

    def freshness(self):
        self.FreshnessNotification.clear()
        Notes = []
        now = datetime.now()
        dt_string = now.strftime("%d/%m/%Y - %H:%M:%S")
        nows = datetime.strptime(dt_string, "%d/%m/%Y - %H:%M:%S")
        
        cursor = self.FreshnessNotification.textCursor()
        cursor.movePosition(QtGui.QTextCursor.End)
        ...
        for things in things.each():
            ...
            if expiry < nows:
                cursor.insertText("Item "+val['Item']+" is not fresh\n")
                self.FreshnessNotification.setTextCursor(cursor)
                self.FreshnessNotification.setTextColor(QtGui.QColor('#FF0000'))
                self.FreshnessNotification.ensureCursorVisible()
                #self.FreshnessNotification.setText("Item "+val['Item']+" is not fresh")
        ...
        self.FreshnessNotification.setTextCursor(cursor)
        self.FreshnessNotification.ensureCursorVisible()
        #self.FreshnessNotification.setText("####### NOTES #######")
        ...
        for i in range(0,len(new_Notes)):
            ...
            self.FreshnessNotification.setTextCursor(cursor)
            self.FreshnessNotification.ensureCursorVisible()
            #self.FreshnessNotification.setText(str(j)+"."+" "+new_Notes[i])
