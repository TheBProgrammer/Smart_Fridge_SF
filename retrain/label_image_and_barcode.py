...
# pip3 mysql.connector.python for mysql.connector lib download 
...
if __name__ == "__main__":
    ...
    for files in os.listdir(folder_path + "test-images"):
        ...
        if(Check_for_barcode(file_name)):
            ...
            qty = val   #input quantity as string
            try:
                mydb = mysql.connector.connect(host="localhost", user="root", passwd="", database="entries")
                if (mydb.is_connected()):
                    mycursor = mydb.cursor()
                    sql = "INSERT INTO storage(ItemName,Quantity,Date,time) VALUES(%s,%s,%s,%s)"
                    val = (item, qty , dateString , time)
